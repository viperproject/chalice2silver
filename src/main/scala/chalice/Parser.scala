...
//  }
  ...
  lexical.reserved += ("class", "ghost", "var", "const", "method", "channel", "condition",
                       ...
                       "seq", "nil", "result", "eval", "token", "empty",
                       "wait", "signal", "unlimited", "set",  "sum",  "max",
                       "refines", "tracked", "transforms", "replaces", "by", "spec", "_", "*"
                      )
  ...
  def fieldDecl = {
    ...
    (
      ("tracked" ?) ~ ("ghost" ?) ~ ("var" ~> idType) <~ Semi ^^ { case tr ~ g ~ ((id,t)) => Field(id.v, t, g.isDefined, tr.isDefined) }
    )
  }
  def invariantDecl = {
    currentLocalVariables = Set[String]();
  ...
  def typeDecl: Parser[Type] =
      positioned(("int" | "bool" | ident | "string" | "seq" | "set") ~ opt("<" ~> repsep(typeDecl, ",") <~ ">") ^^ { case t ~ parameters => Type(t, parameters.getOrElse(Nil)) }
  ...
  def suffixThing: Parser[(Expression => Expression)] =
    ...
    | "." ~> backPointerMemberAccess ^^ {
        case (typeId, fieldId) => { e0: Expression => BackPointerMemberAccess(e0, typeId.v, fieldId.v) } }
  ...
  def backPointerMemberAccess =
    ( "~" ~> "(" ~> Ident ~ "." ~ Ident <~ ")" ^^ {
      case typeId ~ _ ~ fieldId => (typeId, fieldId)
    })
  ...
  def atom : Parser[Expression] =
    ...
    | backPointerMemberAccess ^^ {
        case (typeId, fieldId) =>
          val implicitThis = ImplicitThisExpr(); implicitThis.pos = typeId.pos
          BackPointerMemberAccess(implicitThis, typeId.v, fieldId.v)
      }
    ...
    | "rd" ~> "(" ~>
      ( // note: depending on the context where rd occurs, we parse it wrongly (e.g. acc(x,rd(n)) is parsed to
        ...
        | (((atom <~ ".") ?) ~ backPointerMemberAccess ^^ (_ match {
              case None ~ ((t,i)) => { val result = BackPointerMemberAccess(ImplicitThisExpr(),t.v,i.v); result.pos = t.pos; result }
              case Some(a) ~ ((t,i)) => { val result = BackPointerMemberAccess(a,t.v,i.v); result.pos = t.pos; result }
          })) ~ rdPermArg <~ ")"
    ...
    | "rd" ~> "*" ~> "(" ~>
      ( (( (Ident ^^ (e => { val result = MemberAccess(ImplicitThisExpr(),e.v); result.pos = e.pos; result})) <~ ")"
        ...
        | selectExprFerSureX <~ ")"
          ...
          case (b @ BackPointerMemberAccess(_,_,_)) => BackPointerAccess(b, Star);
          case (m @ MemberAccess(_,_)) => Access(m, Star)
        })
      )
    | "acc" ~> "(" ~>
    ...
    | ("ite" ~> "(" ~> expression <~ ",") ~ (expression <~ ",") ~ (expression <~ ")") ^^ {
        case con ~ thn ~ els => IfThenElse (con, thn, els) }
    | "(" ~> expression <~ ")"
    | quantifierType
    | quantifierSeq
    | quantifierSet
    ...
    | ("empty" ~> "<") ~> (typeDecl <~ ">") ^^ EmptySet
    | ("{" ~> expressionList <~ "}") ^^ { case es => ExplicitSet(es) }
    )
  ...
  def quantifierSet: Parser[Quantification] =
    (quant ~ repsep(ident, ",")) into {case q ~ is => quantifierSetBody(q, is)}
  def quantifierSetBody(q: Quant, is: List[String]) : Parser[Quantification] =
    ((":" ~> expression <~ "::") ~ (exprWithLocals(is))) ^^
      { case set ~ e =>
        currentLocalVariables = currentLocalVariables -- is;
        SetQuantification(q, is, set, e);
      }
  def quant: Parser[Quant] =
    ( "forall" ^^^ Forall | "exists" ^^^ Exists | "sum" ^^^ Sum | "max" ^^^ Max )
