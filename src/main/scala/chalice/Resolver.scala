...
object Resolver {
 ...
 def Resolve(prog: List[TopLevelDecl]): ResolverOutcome = {
   ...
   //  * Call graph for functions (calls)
   //  * Call graph that also considers calls in contracts (calls2)
   val calls = new DiGraph[Function];
   val calls2 = new DiGraph[Function];
   for (decl <- prog) decl match {
     case ch: Channel =>
     ...
     case cl: Class =>
       for (m <- cl.members) {
         ...
         m match {
           ...
           case f@Field(id, tp, _, trk) =>
             ResolveType(tp, context)
             if(trk && (tp.typ==null || !tp.typ.IsNormalClass))
             {
               // if a field is tracked, then it must be a reference
               context.Error(f.pos, "the tracked field " + id + " should have a class type (found " + tp.FullName + ")")
             }
           ...
           case f@Function(id, ins, out, spec, definition) =>
             ...
             def updateCallGraphs(e: Expression, updateCalls: Boolean = false) = {
               e visit {
                 case app : FunctionApplication if app.f != null /* may not be resolved */ =>
                   ...
                   calls2 addNode app.f;
                   calls2.addEdge(f, app.f);
                 case _ =>
               }
             }
             calls addNode f;
             ...
             spec foreach {
               ...
               case p@Postcondition(e) =>
                 ...
                 // The following check is necessary, because the postcondition axiom has the limited function as a trigger.  If we were to allow unfolding expressions, thn this might introduce a matching loop.  Since we don't know of any cases where one would use an unfolding expression in the postcondition, we forbid it here.
   ...
   val (callGraphCondensation, h) = calls2.computeSCC;
 ...
 def ResolveType(t: Type, context: ProgramContext): Unit = {
     ...
     if (context.decls contains t.FullName) {
       ...
       if(t.id.equals("seq") && t.params.length == 1) {
         ...
         if(t.id.equals("set") && t.params.length == 1) {
           t.typ = new SetClass(t.params(0).typ);
         } else {
 ...
 def ResolveExpr(e: RValue, context: ProgramContext,
       ...
       for(ini@Init(f, init) <- initialization) {
         if (fieldNames contains f) {
           ...
           e.typ.LookupMember(f) match {
             case Some(field@Field(name, tp, _, _)) =>
   ...
   case sel @ BackPointerMemberAccess(e, pointingClassId, pointingFieldId) =>
     var c : Class = IntClass
     if(context.decls contains pointingClassId) {
       context.decls(pointingClassId) match {
         case sc:Class => c = sc
         case _ =>
       }
     }
     if(c==null || !c.IsNormalClass) {
       context.Error(e.pos, "Backpointer field must mention a class. Found: " + pointingClassId)
     }
     var typ: Class = IntClass
     ResolveExpr(e, context, twoStateContext, false)
     c.LookupMember(pointingFieldId) match {
       case None => context.Error(e.pos, "There is no field " + pointingFieldId + " in class " + pointingClassId)
       case Some(f:Field) =>
         if(f.typ.typ != e.typ) {
           context.Error(e.pos, "The field " + f.id + " in class " + pointingClassId + " is of type " + f.typ.typ.id + ". Expected: " + e.typ.id)
         }
         else if(!f.isTracked) {
           context.Error(e.pos, "The field " + f.id + " in class " + e.typ.id + " is not tracked")
         }
         typ = new SetClass(c)
       case _ => context.Error(e.pos, "The member " + pointingFieldId + " of class " + e.typ.FullName + " is not a field")
     }
     sel.typ = typ
   case sel @ MemberAccess(e, id) =>
     ResolveExpr(e, context, twoStateContext, false)
   ...
   case expr @ BackPointerAccess(e, perm) =>
   ...
   case expr@ Old(e) =>
     ...
     ResolveExpr(e, context, twoStateContext, false)
     expr.typ = e.typ
   case ite@IfThenElse(con, thn, els) =>
     ResolveExpr(con, context, twoStateContext, false); ResolveExpr(thn, context, twoStateContext, specContext); ResolveExpr(els, context, twoStateContext, specContext);
     if (!con.typ.IsBool) context.Error(con.pos, "condition of if-then-else expression must be a boolean");
     if (!canAssign(thn.typ, els.typ) && !canAssign(els.typ, thn.typ)) context.Error(ite.pos, "the thn and else branch of an if-then-else expression must have compatible types");
     ite.typ = thn.typ;
   case expr@ Not(e) =>
     ResolveExpr(e, context, twoStateContext, false)
   ...
   case contains@Contains(e0, e1) =>
     ResolveExpr(e0, context, twoStateContext, false);
     ResolveExpr(e1, context, twoStateContext, false);
     if(! e1.typ.IsSeq && !e1.typ.IsSet)
       context.Error(contains.pos, "RHS operand of 'in' must be sequence or set. (found: " + e1.typ.FullName + ").");
     else if(! canAssign(e1.typ.parameters(0), e0.typ))
       context.Error(contains.pos, "LHS operand's type must be element type of sequence or set. (found: " + e0.typ.FullName + ", expected: " + e1.typ.parameters(0).FullName + ").");
   ...
   case bin: BinaryExpr =>
     ...
     if (bin.mayBeSetOperation && bin.E1.typ.id=="set") {
       // check for all binary operations except containment: the types of the two expressions must be the same
       if(bin.E0.typ!=bin.E1.typ)
         context.Error(bin.pos, bin.OpName + " requires operands of the same type, found " + bin.E0.typ.FullName + " and " + bin.E1.typ.FullName)

       // if it is a comparison, then the result is boolean
       bin.typ = if(bin.ResultType==BoolClass) BoolClass else bin.E0.typ
     }
     else
     {
   ...
   case q: Quantification =>
     ...
     val typ = q match {
       case q: SeqQuantification =>
         ...
         if(! q.seq.typ.IsSeq) {
           context.Error(q.seq.pos, "This quantification / aggregation must range over a sequence. (found: " + q.seq.typ.FullName + ").");
           None;
         } else
           Some(q.seq.typ.parameters(0));
       case q: TypeQuantification =>
         ResolveType(q.t, context);
         if (q.t.typ == null) None
         else Some(q.t.typ)
       case q: SetQuantification =>
         ResolveExpr(q.dom, context, twoStateContext, false);
         if(! q.dom.typ.IsSet) {
           context.Error(q.dom.pos, "This quantification / aggregation must range over a set. (found: " + q.dom.typ.FullName + ").");
           None;
         } else
           Some(q.dom.typ.parameters(0));
     };
     ...
     if (typ.isDefined) {
       ...
       val specContextOfBody = specContext && q.Q==Forall
       ResolveExpr(q.E, bodyContext, twoStateContext, specContextOfBody);
       if((q.Q==Forall || q.Q==Exists) && ! q.E.typ.IsBool)
         context.Error(q.E.pos, "Body of quantification must be a boolean. (found: " +  q.E.typ.FullName + ").");
       if((q.Q==Sum || q.Q==Max) && ! q.E.typ.IsInt)
         context.Error(q.E.pos, "Body of aggregation must be a integer. (found: " +  q.E.typ.FullName + ").");
       q.variables = bvariables;
       q.typ = q.E.typ
     }
     else { q.typ = if (q.Q==Forall || q.Q==Exists) chalice.BoolClass else chalice.IntClass }
   case seq@EmptySeq(t) =>
   ...
   case len@Length(e) =>
     ResolveExpr(e, context, twoStateContext, false);
     if(! e.typ.IsSeq && !e.typ.IsSet) context.Error(len.pos, "The operand of |.| expression must be sequence or set. (found: " + e.typ.FullName + ").");
     len.typ = IntClass;
   case set@EmptySet(t) =>
     ResolveType(t, context)
     set.typ = SetClass(t.typ)
   case set@ExplicitSet(es) =>
     es foreach { e => ResolveExpr(e, context, twoStateContext, false) }
     es match {
       case Nil => set.typ = SetClass(IntClass)
       case h :: t =>
         t foreach { e => if(! (e.typ == h.typ)) context.Error(e.pos, "The elements of the set expression have different types.")};
         set.typ = SetClass(h.typ);
     }
   case ev@Eval(h, e) =>
 ...
 def CheckRunSpecification(e: Expression, context: ProgramContext, allowMaxLock: Boolean): Unit = e match {
   ...
   case q: TypeQuantification =>
     CheckRunSpecification(q.e, context, true)
   case q: SetQuantification =>
     CheckRunSpecification(q.dom, context, false)
     CheckRunSpecification(q.body, context, true)
   case Length(e) =>
     CheckRunSpecification(e, context, false);
   case ExplicitSeq(es) =>
