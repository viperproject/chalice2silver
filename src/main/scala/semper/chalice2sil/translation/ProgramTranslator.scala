...
class ProgramTranslator(val programOptions: semper.chalice2sil.ProgramOptions, val programName: String)
{
  ...
  // maps Chalice entities (fields, methods, functions, predicates) to the corresponding SIL entities
  ...
  // maps Chalice class members to SIL members and local variables
    // todo: refactor/fix: use only silEnvironment for this mapping
  ...
  // name generator -- ensures uniqueness and validity of names of identifiers
    ...
    // todo: explore if this convention works in the presence of nested code blocks
  ...
  protected def collectSymbols(decl : chalice.TopLevelDecl) {
    decl match {
      ...
      // only classes are supported as top-level declarations at the moment
...
/*        if(typ.params.length != 1) { messages += WrongNumberOfTypeParameters; Int }
  ...
  protected def translateVars(cVars: Seq[chalice.Variable]) = {
    ...
    cVars.foreach((x: chalice.Variable) => result += LocalVarDecl(x.id, translateType(x.t))(
       SourcePosition(null, x.pos.line, x.pos.column)
    ))
    result.toList
  }
