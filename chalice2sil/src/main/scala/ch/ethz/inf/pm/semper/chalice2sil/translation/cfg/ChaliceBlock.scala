...
class ChaliceBlock(val name : String) { origin =>
  ...
  private[cfg] var _immediateDominator : ChaliceBlock  = null
  ...
  private[this] val _ϕ = mutable.Map[chalice.Variable,Set[ChaliceBlock]]()

  /**
    * The ϕ-assignments for variable `v`, expressed as the set of blocks that contribute
    * a variable version.
    *
    ...
    * @param v The variable for which to return the argument list of the ϕ assignment.
    */
  def ϕ(v : chalice.Variable) = _ϕ.getOrElseUpdate(v,Set[ChaliceBlock]())
